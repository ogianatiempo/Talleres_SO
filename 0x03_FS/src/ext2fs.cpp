...
{
	//TODO: Ejercicio 2
	struct Ext2FSInode *inodo = (struct Ext2FSInode *) malloc(sizeof(Ext2FSInode));

	unsigned int blockGroup_index = blockgroup_for_inode(inode_number);
	Ext2FSBlockGroupDescriptor *blockGroupDescriptor = block_group(blockGroup_index);
	...
	unsigned int inodeBlockNumber = (inodeIndex * inodeSize) / blockSize;

	struct Ext2FSInode *inodeTable = (struct Ext2FSInode *) malloc(1024 << _superblock->log_block_size);
	
	read_block(blockGroupDescriptor->inode_table + inodeBlockNumber, (unsigned char *) inodeTable);

	*inodo = inodeTable[inodeIndex - inodeBlockNumber * (blockSize / inodeSize)];
...
unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	...
	if (block_number < 13) {
		// Directo
		address = inode->block[block_number];
	...
	if (block_number < 2 << 7) {
		// Indirecto
		unsigned int *tabla = (unsigned int *) malloc(1024 << _superblock->log_block_size);
		read_block(inode->block[13], (unsigned char *) tabla);

		address = tabla[block_number];
	...
	if (block_number < 2 << 14) {
		// Doble indirecto
		unsigned int *primerTabla = (unsigned int *) malloc(1024 << _superblock->log_block_size);
		read_block(inode->block[14], (unsigned char *) primerTabla);

		unsigned int *segundaTabla = (unsigned int *) malloc(1024 << _superblock->log_block_size);
		read_block(primerTabla[block_number >> 7], (unsigned char *) segundaTabla);

		address = segundaTabla[block_number & ((2 << 7) - 1)];
	...
	if (block_number < 2 << 21) {
		// Triple indirecto
		unsigned int *primerTabla = (unsigned int *) malloc(1024 << _superblock->log_block_size);
		read_block(inode->block[15], (unsigned char *) primerTabla);

		unsigned int *segundaTabla = (unsigned int *) malloc(1024 << _superblock->log_block_size);
		read_block(primerTabla[block_number >> 14], (unsigned char *) segundaTabla);

		block_number &= ((2 << 14) - 1);

		unsigned int *tercerTabla = (unsigned int *) malloc(1024 << _superblock->log_block_size);
		read_block(segundaTabla[block_number >> 7], (unsigned char *) tercerTabla);

		address = tercerTabla[block_number & ((2 << 7) - 1)];
...
struct Ext2FSInode * Ext2FS::get_file_inode_from_dir_inode(struct Ext2FSInode * from, const char * filename)
{
	...
	printf("Busco el dir: %s\n", filename);
	//TODO: Ejercicio 3
		
	unsigned int blockSize = 1024 << _superblock->log_block_size;
	unsigned char *dirEntryTable = (unsigned char *) malloc(2 * blockSize);
	unsigned int blockAddress;
	unsigned char *currentEntry;
	unsigned int currentEntrySize;

	for(unsigned long i = 0; i < from->blocks - 1; i++){	
		if (i == 0) {
			// La primera vez leo dos bloques y apunto currentEntry al principio
			printf("dirEntryTable address: %p\n", dirEntryTable);
			printf("dirEntryTable sencond part address: %p\n", dirEntryTable + blockSize);
			blockAddress = get_block_address(from,i);
			read_block(blockAddress, dirEntryTable);
			blockAddress = get_block_address(from,i+1);
			read_block(blockAddress, dirEntryTable + blockSize);
			currentEntry = dirEntryTable;
		} else {
			// Sino paso el segundo bloque al lugar del primero y leo un nuevo segundo.
			// Retraso currentEntry para que apunte a donde me quedé.
			memcpy(dirEntryTable, dirEntryTable + blockSize, blockSize);
			blockAddress = get_block_address(from,i+1);
			read_block(blockAddress, dirEntryTable + blockSize);
			currentEntry -= blockSize;
		}
		printf("Lei los bloques.\n");

		// Cada vez que arranco estoy dentro del primer bloque,
		// entonces el primer dirEntry siempre está entero.
		bool dirEntryEntero = true;
		while(dirEntryEntero){
			printf("currentEntry address: %p\n", currentEntry);
			printf("Listando: %s\n", ((struct Ext2FSDirEntry*)currentEntry)->name);
			if (strncmp(filename, ((struct Ext2FSDirEntry*)currentEntry)->name,((struct Ext2FSDirEntry*)currentEntry)->name_length) == 0) {
				// Si es el directorio que busco, cargo su inodo y devuelvo el puntero
				printf("Es el directorio que busco.\n");
				unsigned int inodo = ((struct Ext2FSDirEntry*)currentEntry)->inode;
				free(dirEntryTable);
				return load_inode(inodo);
			};

			// Sino actualizo current entry y veo si está completa
			printf("No es el directorio que busco.\n");
			currentEntry += ((struct Ext2FSDirEntry*)currentEntry)->record_length;
			currentEntrySize = ((struct Ext2FSDirEntry*)currentEntry)->record_length;
			if (currentEntry + currentEntrySize >= dirEntryTable + 2 * blockSize) {
				// Si no esta completa salgo
				printf("No lo encontre en estos dos bloques.\n");
				dirEntryEntero = false;
			};
		}
	}
	// Si recorri todos los bloques y no lo encontre devuelvo NULL
	free(dirEntryTable);
	return NULL; 
}
